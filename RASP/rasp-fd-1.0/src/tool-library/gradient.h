...
private:
  ...
  rgb interpolate(rgb c1, rgb c2, float normalized_value){
    ...
    int red = (int)((1.0-normalized_value)*c1.red + normalized_value*c2.red);
    int green = (int)((1.0-normalized_value)*c1.green + normalized_value*c2.green);
    int blue = (int)((1.0-normalized_value)*c1.blue + normalized_value*c2.blue);
...
public:
   ...
   */
//  void initialize(uint16_t min, uint16_t max, std::vector<rgb> stops, rgb minOutlierColor=INVALID_COLOR, rgb maxOutlierColor=INVALID_COLOR){
  void initialize(float min, float max, std::vector<rgb> stops, rgb minOutlierColor=INVALID_COLOR, rgb maxOutlierColor=INVALID_COLOR){
   ...
   */
//  Gradient(uint16_t min, uint16_t max, std::vector<rgb> stops,rgb minOutlierColor=INVALID_COLOR, rgb maxOutlierColor=INVALID_COLOR){
  Gradient(float min, float max, std::vector<rgb> stops,rgb minOutlierColor=INVALID_COLOR, rgb maxOutlierColor=INVALID_COLOR){
    ...
    rgb getRgb(float value){
      // Handle outliers
		if( value < m_min ){
			return m_minOutlierColor == INVALID_COLOR ? m_stops.front() : m_minOutlierColor;
		}
		if( value > m_max ){
			return m_maxOutlierColor == INVALID_COLOR ? m_stops.back()  : m_maxOutlierColor;
		}
      ...
      //float normalized_v = v/range;
      
      //std::cerr << normalized_v << " " << bin<<std::endl;
